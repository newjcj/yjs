...
class util_curl {
    ...
    function vpost($url,$data){ // 模拟提交数据函数
        ...
        if ($this->cookies == TRUE) curl_setopt($curl, CURLOPT_COOKIEFILE, $this->cookie_file);//包含cookie数据的文件名，cookie文件的格式可以是Netscape格式，或者只是纯HTTP头部信息存入文件。 
        if ($this->cookies == TRUE) curl_setopt($curl, CURLOPT_COOKIEJAR, $this->cookie_file);//连接结束后保存cookie信息的文件。
...
$shopUser['password'] = 'g17869098';
 ...
 // $cookieFile = 'D:/WWW/cook.txt';
 $cookieFile = './cookie_aaa.txt';
 //C:\Users\Administrator\AppData\Roaming\Microsoft\Windows\Cookies\jd
// $cookieFile = tempnam("./temp","cookie");//设置cookie的保存位置
// file_put_contents($cookieFile,"");
$http = new util_curl(true,$cookieFile);
 
//1.获取登录页面代码，从中找出uuid变量值
// $showLoginUrl = "https://pop.jd.com/userLoginAction.action?time=".time();
...
?>
<form action="./fivep.php" method="post">
...
<input type="text" name="uuid" value="<?php echo $uuid;?>"><br>
<input type="text" name="eid" value="<?php echo $eid;?>"><br>
<input type="text" name="tid" value="<?php echo $tid;?>"><br>
<input type="text" name="fp" value="<?php echo $fp;?>"><br>
<input type="text" name="name_ch" value="<?php echo $name_ch;?>"><br>
<input type="text" name="value_ch" value="<?php echo $value_ch;?>"><br>
