        ...
        <div class="page-container">
            ...
            <div class="page-content-wrapper">
                ...
                <div class="page-content">
                    <div class="portlet ">
                        <form class="form-inline" action="<?=site_url('yjs/related_cate_host_word');?>" method="get" id="dateform">
                          ...
                          <div class="form-group">
                            <label class="sr-only" for="exampleInputEmail3"></label>
                            <input type="text" class="form-control" id="exampleInputEmail3" name="key" value="<?php echo $this->input->get('sku')?>" placeholder="输入关键字">
                          </div>
                        ...
                        <button type="button" class="btn btn-default navbar-btn" onclick="gtitle1()">行业热词TOP榜单</button>
                        <button type="button" class="btn btn-default navbar-btn" onclick="gtitle2()">行业热词飙升榜单</button>
                        <script>
                            function gtitle1(){
                        ...
                        <div class="portlet box green" data='cc' id="g1">
                                    ...
                                    <div class="portlet-body">
                                        <form action="<?=site_url('datainto/related_cate_host_word')?>" method="post" id="formdata">
                                        <table class="table table-striped table-bordered table-hover" id="sample_1">
                                            ...
                                            <tbody>
                                                <?php foreach($data['detail']['HotWords']['value'] as $key=>$value){?>
                                                    <tr id="tr_db">
                                                        <input type="hidden" value="<?php echo $key+1;?>" name="key[]" />  <!-- 序号 -->
                                                        <input type="hidden" value="<?php echo $data['detail']['HotWords']['value'][$key];?>" />    <!-- 热词 --> 
                                                        <input type="hidden" value="<?php echo $data['detail']['HotWordsNoLink']['value'][$key];?>" />    <!-- 没链接的热词 --> 
                                                        <input type="hidden" value="<?php echo $data['detail']['SearchPopularity']['value'][$key]?>"  />    <!-- 搜索人气 --> 
                                                        <input type="hidden" value="<?php echo $data['detail']['SearchIndex']['value'][$key]?>"  />    <!-- 搜索指数 --> 
                                                        <input type="hidden" value="<?php echo $data['detail']['ClickIndex']['value'][$key]?>"  />    <!-- 点击指数 -->
                                                        <input type="hidden" value="<?php echo $data['detail']['ClickRate']['value'][$key]?>" name="prosku[]" />    <!-- 点击率 -->
                                                        <input type="hidden" value="<?php echo $data['detail']['SaleIndex']['value'][$key]?>" name="prosku[]" />    <!-- 成交指数 -->
                                                        <input type="hidden" value="<?php echo $data['detail']['ConvertRate']['value'][$key]?>" name="prosku[]" />    <!-- 转化率 -->
                                                        
                                                        <input type="hidden" value=" <?php echo date("Y-m-d H:m:s",time())?>" name="prosku[]" />    <!-- 时间 -->
                                                        
                                                        
                                                        <td><?php echo $key+1;?></td> 
                                                        <td> <?php echo $data['detail']['HotWords']['value'][$key]?></td>                                                
                                                        <td> <?php echo $data['detail']['SearchPopularity']['value'][$key]?></td>                                                
                                                        <td> <?php echo $data['detail']['SearchIndex']['value'][$key]?></td>                                                
                                                        <td> <?php echo $data['detail']['ClickIndex']['value'][$key]?></td>                                                
                                                        <td> <?php echo $data['detail']['ClickRate']['value'][$key]?></td>                                                
                                                        <td> <?php echo $data['detail']['SaleIndex']['value'][$key]?></td>                                                
                                                        <td> <?php echo $data['detail']['ConvertRate']['value'][$key]?></td>                                                
                        ...
                        <div class="portlet box green" data='cc' id="g2">
                                    ...
                                    <div class="portlet-body">
                                        ...
                                        <table class="table table-striped table-bordered table-hover" id="sample_2">
                                            ...
                                            <tbody>
                                                <?php foreach($data1['detail']['HotWords']['value'] as $key=>$value){?>
                                                    <tr id="tr_db">
                                                        <input type="hidden" value="<?php echo $key+1;?>" name="key[]" />  <!-- 序号 -->
                                                        <input type="hidden" value="<?php echo $data1['detail']['HotWords']['value'][$key];?>" />    <!-- 热词 --> 
                                                        <input type="hidden" value="<?php echo $data1['detail']['HotWordsNoLink']['value'][$key];?>" />    <!-- 热词无link --> 
                                                        <input type="hidden" value="<?php echo $data1['detail']['SearchSoaredAmplitude']['value'][$key];?>" />    <!-- 飙升幅度 --> 
                                                        <input type="hidden" value="<?php echo $data1['detail']['Level3Name']['value'][$key]?>"  />    <!-- 三级类目 --> 
                                                        <input type="hidden" value=""  />    <!-- 相关热词 --> 
                                                        <input type="hidden" value=""  />    <!-- 热词详情 -->
                                                        
                                                        <input type="hidden" value=" <?php echo date("Y-m-d H:m:s",time())?>" name="prosku[]" />    <!-- 时间 -->
                                                        
                                                        
                                                        <td><?php echo $key+1;?></td> 
                                                        <td> <?php echo $data1['detail']['HotWords']['value'][$key]?></td>                                                
                                                        <td style="color:red;"> <?php $pron=$data1['detail']['SearchSoaredAmplitude']['value'][$key]; echo $pron=ceil($pron*10000)/100?>%</td>                                                
                                                        <td> <?php echo $data1['detail']['Level3Name']['value'][$key]?></td>                                                
                                                        <td> 相关热词</td>                                                
                                                        <td>热词详情</td>                                                
                                                        <td><input type="button" value="点击查看" name="chkk[]" onclick="shop_list(this)"></td>  <!-- 属性详情 -->                                                                                               
                                                    </tr>
