...
<body onload="loadfun()">
   ...
   <!-- <iframe style="width:500px;height:200px;" src="./mess/wone.tpl" id="mess" name="wone" ></iframe>-->
    <script>
        ...
        function getmess(){
            Ajax().get("<{$url}>/view/rand/"+Math.random(),function(data){
        ...
        function send(one){
            ...
            //发送
            //alert(one);
            Ajax().post("<{$url}>/save/rand/"+Math.random(),{username:user,content:con});
    ...
    <form name='frm'>
        用户名:<input style="margin-bottom:10px" type="text" name="username" value="<{$smarty.session.name}>"/><br>
